...
(()=>{
  ...
  // IMPORTANT: use querySelector for '#' targets (previous build passed '#' into getElementById, causing nulls)
  const screenHome=qs('#screen-home'), screenModule=qs('#screen-module');
  const startBtn=$('startBtn'), stopBtn=$('stopBtn'), exitBtn=$('exitBtn'), howBtn=$('howBtn');
  const howBox=qs('#module-how'), titleEl=qs('#module-title');
  ...
  // Guard: if critical nodes missing, bail visibly
  if(!screenHome || !screenModule || !canvas){
    console.error('MindGym: missing core DOM nodes.');
    alert('MindGym error: core UI not found. Please refresh the page.');
    return;
  }
  ...
  function sizeCanvas(){
    // If hidden, temporarily measure based on screenModule width
    const host = canvas.parentElement || document.body;
    let w = host.clientWidth || window.innerWidth;
    w = Math.max(300, Math.floor(w*0.95));
    const h = w;
  ...
  Modules.focus=()=>{
    ...
    function draw(){
      ...
      if(scoreEl) scoreEl.textContent=score; if(bestEl) bestEl.textContent=Math.max(state.best.focus,score);
    }
    function tap(e){
  ...
  Modules.hunt=()=>{
    ...
    function draw(){
      ...
      if(scoreEl) scoreEl.textContent=score; if(bestEl) bestEl.textContent=Math.max(state.best.hunt,score);
    }
    function tap(e){
  ...
  Modules.span=()=>{
    ...
    function draw(){ const ctx=canvas.getContext('2d');
      ctx.clearRect(0,0,canvas.width,canvas.height);
      ...
      if(scoreEl) scoreEl.textContent=score; if(bestEl) bestEl.textContent=Math.max(state.best.span,score);
    }
    function tap(e){
      ...
      if(i!==seq[input.length-1]){ state.streak=0; ensureAudio(); beep(180);
  ...
  function openModule(kind){
    ...
    howBox.classList.add('hidden'); sizeCanvas(); current?.draw?.();
  }
